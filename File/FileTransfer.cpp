...
bool FileTransfer::transfer() {
  try {
    ...
    */
    ...
    if (is_directory(source)) {
      if (!sendDir()) throw std::domain_error("Not working. ");
...
>>>>>>> parent of 56398f9... Added comments and code restyle
}
...
=======
  ...
  if (!exists(source))
    throw std::domain_error(
        "The source does not exists. ");  // The directory does not exists.
