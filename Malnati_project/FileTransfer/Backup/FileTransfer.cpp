	...
	try {
		while (success.load() ) {
...

			{
				...
				string relativePath = (relative(p.generic(), source.parent_path().generic())).generic().string() ;
...
vector<string> FileTransfer::exploreDir() {
	...
	for (; rdi != end_rdi; rdi++)
	{
		if (is_directory((*rdi).path()))
			tree.push_back(relative((*rdi).path(), source.parent_path()).generic().string());
	}
	return tree;
}
